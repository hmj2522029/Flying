...
{
    [SerializeField] TitlePortal OppositePortal; //反対側のポータル
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            TitlePlayer player = collision.GetComponent<TitlePlayer>();

            //クールダウン中はテレポートしない
            if (player != null && player.LastPortalT == this) return;
            ...
            if (OppositePortal != null) //反対側のポータルが存在する場合
            {
                ...
                player.LastPortalT = OppositePortal; //最後に入ったポータルを更新
